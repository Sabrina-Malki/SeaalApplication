...
<?php
$j=0;
$req = $db ->prepare(" SELECT DateReponse,TextRep,IDCourrier,Valide,IDService,Commentaires FROM reponse WHERE ID= ?");
$req->execute(array($_GET["id"]));
$donnee = $req->fetch();
$req0 = $db->prepare("SELECT ID,Objet,Expediteur,DateArrive,Commentaires,DateSaisie,IDTypologie,Chemin,IDAuteur,DateLimite FROM courrier WHERE ID = ? ");
    ...
    <div class="col-md-12 col-sm-12 col-lg-6 col-md-offset-3">
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                ...
                <div class="form-group validate-input">
                    <label for="exp">Date de réception:</label>
                    <input  id="exp" name="exp" style="border:none;background-color:transparent;" value="<?php $date1 = new DateTime($donnee0["DateSaisie"]); echo $date1->format('d-m-Y H:i:s'); ?>" READONLY>
                    <span id="error_name" class="text-danger"></span>
                </div>
                ...
                <div class="panel-heading">Réponse du service <?php echo $donnee1["Designation"] ?> </div>
                <div class="form-group validate-input">
                ...
                <div class="form-group validate-input">
                    <label for="exp">Date d'envoie de la réponse :</label>
                ...
                <div class="form-group validate-input">
                    <label for="obj">Commentaire envoyé par le service de DG:</label>
                    <textarea id="cmt" name="cmt"  class="form-control" placeholder="Aucun Commentaire" rows="5" readonly> <?php echo $donnee["Commentaires"] ?></textarea>
                    <span id="error_obj" class="text-danger"></span>
                </div>
                </br>
                <a href="reponse.php?idc=<?php echo $donnee0["ID"] ?>" class="btn btn-primary btn-primary" onclick="window.open(this.href, 'Popup', 'scrollbars=0,resizable=0,height=420,width=400,top=400,left=400'); return false;">Envoyer une autre réponse <span class="glyphicon glyphicon-share-alt"></span></a>
